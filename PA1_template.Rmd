...
adjustedSteps <- steps
  for(i in 1:length(adjustedSteps)) {
    if(is.na(steps[i])) {
      if(i %% 288 == 0) {
        adjustedSteps[i] <- totals[288];
      } else {
        adjustedSteps[i] <- totals[ i %% 288];
      }
    }
  }
  adjustedTotals <- rep(0,288);
  for(i in 1:287) {
    adjustedTotals[i] <- sum(adjustedSteps[indices[which(indices %% 288 == i)]]);
  }
  adjustedTotals[288] <- sum(adjustedSteps[indices[which(indices %% 288 == 0)]]);
  adjustedTotals;
plot(adjustedTotals, type="l")
```
